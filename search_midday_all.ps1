...
try {
    ...
    Write-Host "================ ALL MIDDAY / CALSON PRODUCTS ================"
    while ($reader.Read()) {
        Write-Host "Code: "$reader["InternalCode"] "| Stock: "$reader["PhysicalStock"] "| Cost: "$reader["CurrentCost"] "| Name: "$reader["Name"]
    }
    $connection.Close()
}
catch {
