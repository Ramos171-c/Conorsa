...
for root, dirs, files in os.walk(root_dir):
    if any(x in root for x in [".git", ".vs", "bin", "obj", "publish", "Wpf", "Migrations"]):
        continue
    for file in files:
        if file.endswith(".cs") or file.endswith(".sql") or file.endswith(".ps1"):
            ...
            try:
                ...
                for idx, line in enumerate(lines, 1):
                    if re.search(r'PhysicalStock\s*[-+*=/]', line) or "PhysicalStock =" in line:
                        rel = os.path.relpath(path, root_dir)
                        matches.append((rel, idx, line.strip()))
            except Exception as e:
                pass

print(f"Total core stock mutation lines: {len(matches)}")
