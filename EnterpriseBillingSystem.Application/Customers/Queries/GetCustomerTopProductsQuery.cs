...
public record GetCustomerTopProductsQuery(Guid CustomerId, int Limit = 10) : IRequest<IEnumerable<ProductDto>>;

public class GetCustomerTopProductsQueryHandler : IRequestHandler<GetCustomerTopProductsQuery, IEnumerable<ProductDto>>
{
    private readonly IProductRepository _productRepository;

    public GetCustomerTopProductsQueryHandler(IProductRepository productRepository)
    {
        _productRepository = productRepository;
    }

    public async Task<IEnumerable<ProductDto>> Handle(GetCustomerTopProductsQuery request, CancellationToken cancellationToken)
    {
        var products = await _productRepository.GetTopProductsForCustomerAsync(request.CustomerId, request.Limit, cancellationToken);
